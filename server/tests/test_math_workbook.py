...
class MathWorkbookTests(unittest.TestCase):
    ...
    def test_migration_is_repeatable_and_legacy_upgrade(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(upgrade(str(db)), 11); self.assertEqual(upgrade(str(db)), 11)
            with sqlite3.connect(db) as check:
                self.assertEqual(check.execute("SELECT MAX(version) FROM schema_migrations").fetchone()[0], 11)
