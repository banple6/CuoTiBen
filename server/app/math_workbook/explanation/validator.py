...
def validate_explanation(data: dict, input_data: dict) -> dict:
    ...
    if not isinstance(binding, dict) or {"problem_id", "source_revision", "candidate_solution_result_id", "verification_report_id", "input_hash"} - set(binding):
        _fail("MODEL_RESPONSE_TRACE_MISMATCH")
    if binding.get("problem_id") != input_data.get("problem_id") or binding.get("source_revision") != input_data.get("source_revision") or not all(isinstance(binding.get(key), str) and binding.get(key) for key in ("candidate_solution_result_id", "verification_report_id", "input_hash")):
        _fail("MODEL_RESPONSE_TRACE_MISMATCH")
