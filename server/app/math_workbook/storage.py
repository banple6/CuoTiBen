...
class MathWorkbookStore:
    ...
    def update_formula(self,formula_id:str,changes:dict)->dict:
        ...
        current=self.get_formula(formula_id)
        if not current: raise KeyError(formula_id)
        with self._connect() as db:
        ...
        return self.get_formula(formula_id) or {}

    def update_region(self,region_id:str,changes:dict)->dict:
        ...
        return self.get_region(region_id) or {}
    ...
    def get_import(self,i:str)->dict|None:
        with self._connect() as db:
            row=db.execute("SELECT * FROM math_imports WHERE id=?",(i,)).fetchone()
            if not row:return None
            out=self._decode(dict(row));out['pages']=[self.get_page(x['id']) for x in db.execute("SELECT id FROM math_pages WHERE import_id=? ORDER BY page_index",(i,))];return out
    def get_page(self,i:str)->dict|None:
        with self._connect() as db:
            r=db.execute("SELECT * FROM math_pages WHERE id=?",(i,)).fetchone()
