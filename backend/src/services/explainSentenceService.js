...
export function buildExplainSentencePrompt({ title, sentence, context, paragraph_theme, paragraph_role, question_prompt }) {
  ...
  return [
    "只输出合法 JSON 对象，不要 Markdown、代码块、标题或额外解释。用中文讲解，必要时保留英语术语。",
    "任务：像英语教授一样讲清这一句的主干、修饰层级、忠实翻译、易错点和考题改写路线。",
    "必须包含且不能缺字段：original_sentence,evidence_type,sentence_function,core_skeleton,chunk_layers,grammar_focus,faithful_translation,teaching_interpretation,natural_chinese_meaning,contextual_vocabulary,misreading_traps,exam_paraphrase_routes,simpler_rewrite,simpler_rewrite_translation,mini_check,hierarchy_rebuild,syntactic_variation。",
    "evidence_type 只能取 background_info / transition_signal / core_claim / supporting_evidence / counter_argument / conclusion_marker。",
    "core_skeleton={subject,predicate,complement_or_object}，只写真实主干成分，不要 [subject:] 标记。",
    "chunk_layers 数组项={text,role,attaches_to,gloss}，role 用：核心信息/前置框架/后置修饰/补充说明/让步或条件框架。",
    "grammar_focus 数组保留 1-3 项，每项={phenomenon,function,why_it_matters,title_zh,explanation_zh,why_it_matters_zh,example_en}。",
    "contextual_vocabulary 数组项={term,meaning}，只列本句关键义。",
    "faithful_translation 是忠实翻译；teaching_interpretation 是教学解读；natural_chinese_meaning 与教学解读一致。",
    "misreading_traps 写主干/修饰/指代/逻辑的误读风险；exam_paraphrase_routes 写同义替换、因果偷换、范围变化等命题路线。",
    "simpler_rewrite 用简单英语保留原意；simpler_rewrite_translation 用中文解释简化方式；mini_check 给一个短问题。",
    "hierarchy_rebuild 用数组分层重组长难句，简单句返回空数组；syntactic_variation 给更易懂句式。",
    "信息不足时填空字符串或空数组，仍保持字段完整。",
    "",
    `资料标题: ${safeTitle}`,
    `句子: ${sentence.trim()}`,
    `上下文: ${safeContext}`,
    ...
    "",
    "优先顺序：句子定位→主干→语块→语法点→易错点→考题改写→简化改写→微练习。"
  ].join("\n");
}
