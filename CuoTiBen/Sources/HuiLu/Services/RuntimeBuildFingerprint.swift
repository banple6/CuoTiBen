...
struct RuntimeBuildFingerprint: Equatable {
    ...
    static var current: RuntimeBuildFingerprint {
        ...
        return RuntimeBuildFingerprint(
            gitSHA: fingerprintString("GIT_SHA") ?? infoString("GIT_SHA") ?? infoString("GitSHA") ?? sourceGitSHA,
            branchName: fingerprintString("GIT_BRANCH") ?? infoString("GIT_BRANCH") ?? infoString("GitBranch") ?? sourceBranchName,
            buildTime: fingerprintString("BUILD_TIME") ?? executableBuildTime() ?? infoString("BUILD_TIME") ?? sourceGeneratedAt,
            appConfiguration: fingerprintString("APP_CONFIGURATION") ?? infoString("APP_CONFIGURATION") ?? defaultConfiguration,
    ...
    private static func fingerprintString(_ key: String) -> String? {
        guard let url = Bundle.main.url(forResource: "BuildFingerprint", withExtension: "plist"),
